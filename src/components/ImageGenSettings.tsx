...
export const useEffectivePresets = () => {
  ...
  const allSavedPrompts = useAppStore((s) => s.savedPrompts);
  const selectedSavedPromptIds = useAppStore((s) => s.selectedSavedPromptIds);
  const savedPrompts = allSavedPrompts.filter((p) => p.kind === 'image');
  ...
  // against stale ids that were deleted by an admin since last load, and
  // against ids left over from before a prompt was switched to video.
...
export const ImageGenSettings = () => {
  ...
  // The library is one shared table across both generators; this panel only
  // ever offers the image half.
  const savedPrompts = allSavedPrompts.filter((p) => p.kind === 'image');
